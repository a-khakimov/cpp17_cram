...
csignal fourier_transform(const csignal& s, bool back = false)
{
    ...
    std::transform(num_iterator{0}, num_iterator{s.size()}, t.begin(), to_ft);
    return t;
}
...
static void print_signal(const csignal& s)
{
    ...
    std::transform(s.begin(), s.end(), std::ostream_iterator<double> { std::cout, " " }, real_val );
    std::cout << std::endl;
}
...
int main()
{
    ...
    std::fill(std::next(trans_sqw.begin(), 10), std::prev(trans_sqw.end(), 10), 0);
